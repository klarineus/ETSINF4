...
public class ProyectoEvaluator implements Evaluator<ArrayList<Integer>>
{
	public Objectives evaluate (ArrayList<Integer> phenotype)
	{
		...
		vacunas.add(1);vacunas.add(2);vacunas.add(0);vacunas.add(3);
		...
		int jovensVac3 = Collections.frequency(jovenes, 3);
		...
		int adults3 = Collections.frequency(adultos, 3);
		...
		int majors3 = Collections.frequency(mayores, 3);
				
		//if(!jovenes.containsAll(vacunas) ||!adultos.containsAll(vacunas) || !mayores.containsAll(vacunas)) {
		if(jovensVac1<1 || jovensVac2<1 || jovensVac0 < 1 ||jovensVac3 < 1|| majors0< 1 || majors1<1 
				|| majors2<1 || majors3<1|| adults1<1 || adults0<1 || adults2 < 1 ||adults3 < 1 || 
				phenotype.get(3) != 3) {
		...
		else 
		{
			...
			for (int id = 0; id < phenotype.size(); id++)
			{
				...
				if(phenotype.get(id)!= 2 && phenotype.get(id)!= 3) numVoluntaris += DatosVacunas.numeroVoluntarios[id];
			}
		}
